...
¿Te gustaría que verifiquemos si el costo se ajusta al presupuesto de costos unificados?`,
  ...
  return (
    <div id="app-root" className="grain min-h-dvh bg-bg text-stone-800 flex flex-col font-sans selection:bg-arza-500 selection:text-white">
      {/* Toast alert overlay */}
      <AnimatePresence>
        {successToast && (
          <motion.div 
            ...
            className="fixed top-5 right-5 z-50 bg-arza-600 text-white font-semibold px-5 py-3 rounded-xl shadow-lg flex items-center space-x-2 border border-arza-500"
          >
      ...
      {/* Main Bar / Header */}
      <header className="bg-gradient-to-r from-arza-900 to-arza-800 border-b border-arza-800 px-5 py-4 shrink-0 shadow-sm text-white rounded-b-xl max-w-7xl w-full mx-auto mt-2 select-none">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="flex items-center space-x-3.5">
            <div className="w-11 h-11 bg-white/10 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/20 shadow-md">
              <FileSpreadsheet className="w-6 h-6 text-arza-300 font-black animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] bg-arza-600/60 text-arza-200 border border-arza-400/40 px-2.5 py-0.5 rounded-full font-bold">
                  Sincronizado con Drive y Sheets
                </span>
              </div>
              <p className="text-xs text-arza-200/90 font-medium">Control de Obra y Audición Directa para Rossy Lares Morales</p>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 w-full sm:w-auto justify-end">
            <div className={`px-3 py-1.5 rounded-lg border text-xs font-semibold flex items-center space-x-1.5 ${
              isSandbox 
                ...
                : 'bg-arza-500/15 text-arza-200 border-arza-500/25'
            }`}>
              <span className={`w-2.5 h-2.5 rounded-full ${isSandbox ? 'bg-amber-500' : 'bg-arza-400'} animate-ping`} />
            ...
            {isSandbox ? (
              <button 
                ...
                className="gsi-material-button text-xs font-semibold shrink-0 cursor-pointer overflow-hidden border border-stone-200 bg-white hover:bg-stone-50 text-stone-900 transition-all rounded-lg flex items-center space-x-2 px-3 py-1.5 h-8"
              >
                {isLoggingIn ? (
              ...
              <div className="flex items-center space-x-2 text-white">
                <div className="flex items-center space-x-2 bg-arza-800/60 px-3 py-1 rounded-lg border border-arza-700/50">
                  <User className="w-3.5 h-3.5 text-arza-300" />
                  <span className="text-xs font-semibold text-stone-100 truncate max-w-[120px]">{user?.displayName || "Rossy"}</span>
                </div>
                <button 
      ...
      <section className="bg-transparent max-w-7xl w-full mx-auto mt-4 px-1 select-none animate-fadeIn">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-surface p-4 rounded-xl border border-border shadow-2xs flex items-center space-x-3 md:space-x-3.5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
            <div className="w-10 h-10 bg-arza-50 border border-arza-100 rounded-xl flex items-center justify-center text-arza-600 shrink-0">
              <ShieldCheck className="w-5 h-5 text-arza-600" />
            </div>
            <div>
              <p className="text-[9px] font-bold text-stone-500 uppercase tracking-wider">Costo Real Aprobado</p>
              <h4 className="text-md md:text-lg font-black text-arza-800 font-mono">
                ${trueAuditedCost.toLocaleString("es-MX", { minimumFractionDigits: 2 })}
              </h4>
              {overchargeLeak + deliveryShortageLeak > 0 && (
                <span className="text-[9.5px] text-stone-400 font-medium block leading-none mt-0.5">
          ...
          <div className="bg-surface p-4 rounded-xl border border-border shadow-2xs flex items-center space-x-3 md:space-x-3.5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${
              (overchargeLeak + deliveryShortageLeak) > 0 
                ...
                : 'bg-arza-50 border-arza-100 text-arza-600'
            }`}>
              <TrendingDown className="w-5 h-5" />
            </div>
            <div>
              <p className="text-[9px] font-bold text-stone-500 uppercase tracking-wider">Fuga y Faltante Detectado</p>
              <h4 className={`text-md md:text-lg font-black font-mono ${(overchargeLeak + deliveryShortageLeak) > 0 ? 'text-rose-600' : 'text-arza-600'}`}>
                -${(overchargeLeak + deliveryShortageLeak).toLocaleString("es-MX", { minimumFractionDigits: 2 })}
              </h4>
              <span className="text-[9px] text-stone-400 font-medium block leading-none mt-0.5">
          ...
          <div className="bg-surface p-4 rounded-xl border border-border shadow-2xs flex items-center space-x-3 md:space-x-3.5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
            ...
            <div>
              <p className="text-[9px] font-bold text-stone-500 uppercase tracking-wider">Alertas de Auditoría</p>
              <h4 className="text-md md:text-lg font-black text-stone-800 font-mono">
          ...
          <div className="bg-surface p-4 rounded-xl border border-border shadow-2xs flex items-center space-x-3 md:space-x-3.5 hover:shadow-md hover:-translate-y-0.5 transition-all duration-200">
            ...
            <div>
              <p className="text-[9px] font-bold text-stone-500 uppercase tracking-wider">Catálogo Unificado</p>
              <h4 className="text-md md:text-lg font-black text-stone-800 font-mono">{materialsCount} insumos</h4>
              <span className="text-[9px] text-stone-400 font-medium block leading-none mt-0.5">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full flex flex-col lg:flex-row gap-6 p-4 overflow-hidden">
        ...
        <section id="chat-panel" className="w-full lg:w-5/12 bg-white border border-stone-200 flex flex-col rounded-2xl overflow-hidden h-[680px] shadow-sm">
          <div className="bg-stone-50 border-b border-stone-200 px-4 py-3 shrink-0 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="w-2.5 h-2.5 bg-arza-500 rounded-full animate-pulse" />
              <span className="text-xs font-bold text-stone-700 uppercase tracking-wider">Agente de Conversión Arza</span>
            </div>
            <button 
              onClick={() => {
                setMessages(prev => [
              ...
              className="text-[10px] bg-white px-2.5 py-1.5 rounded-lg inline-flex items-center hover:bg-stone-50 text-stone-500 border border-stone-200 hover:text-stone-700 transition-colors cursor-pointer shadow-2xs"
            >
              Borrar Charla
            </button>
          </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-stone-50/45 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent">
            {messages.map((msg) => (
              <div 
                key={msg.id} 
              ...
              >
                {msg.role === 'assistant' && (
                  <div className="w-7 h-7 bg-arza-600 rounded-lg flex items-center justify-center text-white text-xs shrink-0 self-start shadow-2xs border border-arza-500">
                    <Sparkles className="w-3.5 h-3.5" />
                  </div>
                )}
                <div className={`max-w-[85%] rounded-2xl p-3.5 text-xs leading-relaxed shadow-xs ${
                  msg.role === 'user' 
                    ? 'bg-white border border-stone-200 text-stone-800 font-medium' 
                    : 'bg-arza-50/75 border border-arza-100 text-stone-800'
                }`}>
                  {/* Standard text parser */}
                  <div className="whitespace-pre-line prose prose-xs text-stone-700 font-medium">
                    {msg.content}
                  </div>

                  {/* Executable agent recommendations trigger buttons */}
                  {msg.action && (
                    <div className="mt-3.5 pt-3 border-t border-arza-100/50">
                      <div className="bg-white p-2.5 rounded-xl border border-arza-200 text-[11px] mb-2 shadow-2xs">
                        <p className="font-bold text-arza-700 mb-1 flex items-center">
                          <CheckCircle2 className="w-3.5 h-3.5 mr-1 text-arza-600" />
                      ...
                      {msg.action.executed ? (
                        <span className="inline-flex items-center text-arza-700 font-bold text-[10px] bg-arza-100 px-2.5 py-1 rounded-full shadow-2xs border border-arza-200">
                          <Check className="w-3 h-3 mr-1 text-arza-600" />
                          Aplicado correctamente en tus Tablas
                        </span>
                      ) : (
                        <button 
                          onClick={() => handleExecuteAction(msg.action)}
                          className="w-full bg-arza-600 text-white font-bold px-3 py-1.5 rounded-lg text-[10.5px] hover:bg-arza-700 shadow-xs transition-colors cursor-pointer"
                        >
                  ...
                  <span className={`block opacity-45 text-[9px] mt-1.5 font-mono ${msg.role === 'user' ? 'text-right text-stone-500' : 'text-stone-500'}`}>
                    {msg.timestamp}
                  </span>
                </div>
              </div>
            ...
            {isGenerating && (
              <div className="flex gap-3 justify-start">
                <div className="w-7 h-7 bg-arza-600 text-white rounded-lg flex items-center justify-center text-xs shrink-0 self-start animate-pulse shadow-sm">
                  <Sparkles className="w-3.5 h-3.5" />
                </div>
                <div className="max-w-[85%] rounded-2xl p-3 bg-white border border-stone-200 text-stone-500 text-xs shadow-2xs flex items-center space-x-2">
                  <span className="flex space-x-1">
                    <span className="w-1.5 h-1.5 bg-arza-600 rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                    <span className="w-1.5 h-1.5 bg-arza-600 rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                    <span className="w-1.5 h-1.5 bg-arza-600 rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                  </span>
          ...
          <div className="px-4 py-3 bg-stone-50 border-t border-stone-200 shrink-0 select-none">
            <p className="text-[10px] text-stone-500 font-bold mb-1.5 uppercase tracking-wide">Rossy, presiona un atajo rápido:</p>
            <div className="flex flex-wrap gap-1.5">
              <button 
                onClick={() => handleApplyPreset("¿Qué materiales no tienen códigos de Margarita asignados?")}
                className="text-[10.5px] bg-white hover:bg-stone-50 text-stone-600 hover:text-arza-700 hover:border-arza-300 font-semibold px-2.5 py-1.5 rounded-lg border border-stone-200 transition-colors shadow-2xs cursor-pointer"
              >
          ...
          <form 
            ...
            className="p-3 bg-stone-50 border-t border-stone-200 shrink-0 flex items-center gap-2"
          >
            <button 
              type="button"
              ...
              className={`p-3 rounded-xl border transition-all flex items-center justify-center shrink-0 cursor-pointer ${
                isMicRecording 
                  ...
                  : 'bg-white hover:bg-stone-100 border-stone-200 text-stone-600 hover:text-stone-800 shadow-2xs'
              }`}
            ...
            <input 
              ...
              className="flex-1 bg-white border border-stone-200 text-stone-800 placeholder-stone-400 text-xs px-3.5 py-3 rounded-xl focus:outline-none focus:border-arza-600 text-xs transition-colors shadow-2xs"
            />

            <button 
              type="submit"
              disabled={!inputMessage.trim() || isGenerating}
              className={`p-3 bg-arza-600 hover:bg-arza-700 text-white rounded-xl flex items-center justify-center shrink-0 transition-all font-semibold cursor-pointer ${
                (!inputMessage.trim() || isGenerating) ? 'opacity-40 cursor-not-allowed' : 'shadow-sm shadow-arza-600/10'
              }`}
            >
        ...
        <section id="visualizer-panel" className="flex-1 bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm flex flex-col h-[680px]">
            ...
            <div className="flex space-x-1.5 p-2">
              ...
              >
                ...
                {pendingOrdersCount > 0 && (
                  <span className="absolute -top-1 -right-1 bg-amber-500 text-stone-900 text-[9px] font-black w-4.5 h-4.5 rounded-full flex items-center justify-center border border-white">
                    {pendingOrdersCount}
                  </span>
                )}
              </button>
            ...
            <AnimatePresence mode="wait">
              ...
              {activeTab === 'dashboard' && (
                <motion.div 
                  ...
                  className="space-y-6"
                >
                  <div className="flex justify-between items-center bg-arza-50/50 p-4 rounded-xl border border-arza-100/55 pb-3">
                    <div>
                      <h3 className="text-sm font-extrabold text-[#0d4f30] uppercase tracking-wider">Centro de Control de Obra</h3>
                      <p className="text-xs text-stone-500">Auditoría consolidada y reporteo de gastos del grupo Arza</p>
                    </div>
                    <span className="text-[10px] text-arza-800 font-extrabold bg-arza-100 border border-arza-200 px-3 py-1 rounded-full uppercase">Auditoría en Tiempo Real</span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 select-none">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs">
                      <h4 className="text-xs font-extrabold uppercase tracking-wider text-stone-700 mb-3 flex items-center">
                        <TrendingUp className="w-4 h-4 mr-1.5 text-arza-600" />
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs">
                      <h4 className="text-xs font-extrabold uppercase tracking-wider text-stone-700 mb-3 flex items-center">
                        <BarChart3 className="w-4 h-4 mr-1.5 text-arza-600" />
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs md:col-span-2 flex flex-col md:flex-row items-center gap-6">
                      <div className="flex-1 space-y-2">
                        <h4 className="text-xs font-extrabold uppercase tracking-wider text-stone-700 flex items-center">
                  ...
                  <div className="bg-white p-5 rounded-2xl border border-stone-200 shadow-xs space-y-4 select-none">
                    <div>
                      <h4 className="text-xs font-extrabold uppercase tracking-wider text-stone-700 flex items-center">
                        <Briefcase className="w-4 h-4 mr-1.5 text-arza-600" />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                      {[
                        { name: "Solum T18", progress: 85, color: "from-arza-500 to-arza-600", bg: "bg-arza-50", text: "text-arza-700", border: "border-arza-100", status: "Fase de Acabados" },
                        { name: "Solum T40", progress: 60, color: "from-cyan-500 to-blue-600", bg: "bg-arza-50", text: "text-arza-700", border: "border-arza-100", status: "Instalación Hidráulica" },
                        { name: "Terra", progress: 95, color: "from-green-500 to-arza-600", bg: "bg-green-50", text: "text-green-700", border: "border-green-100", status: "Conclusión de Obra" },
                        ...
                        <div key={i} className={`p-4 rounded-xl border ${project.border} ${project.bg} space-y-3 shadow-2xs hover:scale-[1.01] transition-all`}>
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-[10px] font-mono text-stone-500 font-bold bg-white px-2 py-0.5 rounded-md border border-stone-200">{project.progress}%</span>
                          </div>
                          
                          <div className="w-full bg-stone-200/55 h-2 rounded-full overflow-hidden">
                          ...
                          <div className="flex justify-between items-center text-[10px] text-stone-500 font-medium">
                            ...
                            <span className="text-[9px] bg-white px-1.5 py-0.5 rounded border border-stone-200 text-stone-500 font-mono">Frente #{i+1}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="bg-stone-50 border border-stone-200 rounded-2xl p-5 space-y-4 shadow-2xs">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pb-3 border-b border-stone-200/80">
                      <div>
                        <h4 className="text-sm font-bold text-stone-800 flex items-center gap-1.5">
                          <FileSpreadsheet className="w-4 h-4 text-arza-600" />
                      ...
                      {!isSandbox && token ? (
                        <div className="flex items-center gap-2">
                          <button
                            ...
                            className="text-[11px] font-semibold bg-arza-600 hover:bg-arza-500 text-stone-900 px-3 py-1.5 rounded-lg transition-colors flex items-center gap-1 cursor-pointer disabled:opacity-50"
                          >
                    ...
                    {isSandbox ? (
                      <div className="space-y-6">
                        <div className="text-center py-6 bg-stone-50 rounded-2xl border border-dashed border-stone-200 space-y-3">
                          <AlertTriangle className="w-8 h-8 text-amber-500 mx-auto animate-bounce" />
                          <div className="space-y-1 px-4">
                            <p className="text-xs font-extrabold text-stone-800">¿Deseas conectar tus hojas reales de Google Sheets?</p>
                          ...
                          <button
                            onClick={handleLogin}
                            className="bg-arza-600 hover:bg-arza-700 text-white text-xs font-bold px-4 py-2.5 rounded-lg cursor-pointer transition-all inline-flex items-center gap-1.5 focus:ring-1 focus:ring-arza-500 shadow-xs"
                          >
                      ...
                      <div className="space-y-6">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="space-y-3 select-none">
                            <p className="text-xs font-bold text-stone-700">Hoja de Trabajo Vinculada:</p>
                            
                            {selectedSpreadsheetId ? (
                              <div className="bg-stone-50 p-3 rounded-xl border border-stone-200 space-y-2">
                                <div className="flex justify-between items-start">
                                  <div>
                                    <p className="text-xs font-bold text-stone-800 truncate max-w-[220px]">{selectedSpreadsheetName}</p>
                                    <p className="text-[10px] text-stone-400 font-mono truncate max-w-[220px]">ID: {selectedSpreadsheetId}</p>
                                  </div>
                                  <span className="text-[9px] bg-arza-100 text-arza-800 px-2 py-0.5 rounded-full font-bold uppercase">Vinculado</span>
                                </div>
                                ...
                                <div className="flex gap-2 pt-1 font-bold">
                                  <button
                                    ...
                                    disabled={isSyncingToSheets}
                                    className="flex-1 py-1.5 px-2.5 bg-white hover:bg-stone-50 text-stone-700 border border-stone-200 rounded-lg text-[10.5px] flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50 transition-all shadow-2xs"
                                  >
                                    {isSyncingToSheets ? (
                                      <RotateCw className="w-3 h-3 animate-spin text-arza-600" />
                                    ) : (
                                      <RotateCw className="w-3 h-3 text-arza-600" />
                                    )}
                                    Importar Datos
                                  </button>
                                  <button
                                    ...
                                    disabled={isSyncingToSheets}
                                    className="flex-1 py-1.5 px-2.5 bg-arza-600 hover:bg-arza-700 text-white rounded-lg text-[10.5px] flex items-center justify-center gap-1 cursor-pointer disabled:opacity-50 transition-all shadow-xs"
                                  >
                                    {isSyncingToSheets ? (
                          ...
                          <div className="space-y-2.5 select-none">
                            <div className="flex justify-between items-center text-xs font-bold text-stone-700">
                              ...
                              {isLoadingSpreadsheets && <RotateCw className="w-3.5 h-3.5 text-arza-600 animate-spin" />}
                            </div>

                            <div className="bg-stone-50 rounded-xl p-2 max-h-[140px] overflow-y-auto border border-stone-200 space-y-1">
                                  ...
                                  <button
                                    ...
                                    className={`w-full text-left text-[11px] px-2.5 py-1.5 rounded-lg transition-all flex items-center justify-between border ${
                                      selectedSpreadsheetId === sheet.id 
                                        ? 'bg-arza-50 border-arza-300 text-arza-800 font-bold shadow-2xs' 
                                        : 'hover:bg-stone-100 bg-white border-stone-200 text-stone-600 hover:text-stone-800'
                                    }`}
                                  >
                                    <span className="truncate max-w-[200px] font-medium">{sheet.name}</span>
                                    <span className="text-[9px] font-mono text-stone-400">ID: {sheet.id.slice(0, 6)}...</span>
                                  </button>
                                ))
                              ) : (
              ...
              {activeTab === 'catalogo' && (
                <motion.div 
                  ...
                  className="space-y-4"
                >
                  <div className="flex flex-col sm:flex-row justify-between gap-3 bg-stone-50 p-4 rounded-xl border border-stone-200 mb-2 select-none">
                    <div>
                      <h3 className="text-sm font-bold text-stone-800 flex items-center">
                        <Database className="w-4 h-4 mr-1.5 text-arza-600" />
                        Catálogo Maestro de Costos Unificados
                      </h3>
                      <p className="text-xs text-stone-500">Estructura madre de precios y códigos pactados para mitigar duplicidades</p>
                    </div>
                    ...
                    <div className="relative">
                      ...
                      <input 
                        ...
                        className="bg-white border border-stone-200 text-stone-800 placeholder-stone-400 text-[11.5px] pl-8.5 pr-3 py-2 rounded-xl focus:outline-none focus:border-arza-500 focus:ring-1 focus:ring-arza-500 w-full sm:w-56 transition-all"
                      />
                    </div>
                  </div>
                ...
                >
                  ...
                  {newOrderForm && (
                    <motion.form 
                      ...
                      className="bg-stone-50 p-4 rounded-2xl border border-arza-200 space-y-3 shrink-0"
                    >
                      <h4 className="text-xs font-black text-arza-800 uppercase tracking-wider flex items-center">
                        <Plus className="w-4 h-4 mr-1.5 text-arza-600" />
                        Formulario de Órdenes de Compra Manual
                      </h4>
                      
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-[11px]">
                        <div>
                          <label className="block text-stone-600 font-bold mb-1">Obra / Destino</label>
                          <select 
                            value={formProject} 
                            onChange={(e) => setFormProject(e.target.value)}
                            className="w-full bg-white border border-stone-200 text-stone-800 rounded-lg p-2 focus:outline-none focus:border-arza-500 font-medium"
                          >
                        ...
                        <div>
                          <label className="block text-stone-600 font-bold mb-1">Material del Catálogo</label>
                          <select 
                        ...
                        <div>
                          <label className="block text-stone-600 font-bold mb-1">Cantidad Requerida</label>
                          <input 
                            type="number" 
                            value={formQuantity} 
                            onChange={(e) => setFormQuantity(Number(e.target.value))}
                            className="w-full bg-white border border-stone-200 text-stone-800 rounded-lg p-2 focus:outline-none focus:border-arza-500 font-mono font-bold"
                          />
                        </div>

                        <div>
                          <label className="block text-stone-600 font-bold mb-1">Proveedor Asignado</label>
                          <select 
                            value={formSupplier} 
                      ...
                      <div className="flex justify-end space-x-2 pt-2 text-[10.5px]">
                        <button 
                          ...
                          className="bg-stone-200 hover:bg-stone-300 text-stone-700 font-bold px-3 py-1.5 rounded-lg cursor-pointer transition-all"
                        >
                          Cancelar
                        </button>
                        <button 
                          type="submit"
                          className="bg-arza-600 hover:bg-arza-700 text-white font-bold px-4 py-1.5 rounded-lg cursor-pointer transition-all shadow-xs"
                        >
                  ...
                  <div className="flex flex-col sm:flex-row justify-between gap-3 bg-stone-50 p-4 rounded-xl border border-stone-200 shrink-0 select-none">
                    <div>
                      <h3 className="text-sm font-bold text-stone-800 flex items-center">
                        <FileSpreadsheet className="w-4 h-4 mr-1.5 text-arza-600" />
                        Registro General de Órdenes de Compra (OC)
                      </h3>
                      <p className="text-xs text-stone-500">Padrón de insumos despachados para control y auditoría de Margarita</p>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <select 
                        ...
                        className="bg-white border border-stone-200 text-stone-700 text-[11.5px] font-bold px-3.5 py-1.5 rounded-xl focus:outline-none focus:border-arza-500 transition-all cursor-pointer"
                      >
              ...
              {activeTab === 'bodega' && (
                ...
                >
                  <div className="bg-stone-50 p-4 rounded-xl border border-stone-200 shrink-0 select-none">
                    <h3 className="text-sm font-bold text-stone-800 flex items-center">
                      <FileText className="w-4 h-4 mr-1.5 text-arza-600" />
                      Entradas Auxiliares a Bodega (Almacenamiento)
                    </h3>
                    <p className="text-xs text-stone-500">Verificaciones de entrega y volumetría cargadas por Joli y Kari</p>
                  </div>
          ...
          <footer className="bg-stone-50 border-t border-stone-200 px-5 py-3 text-[10.5px] text-stone-500 flex flex-col sm:flex-row justify-between items-center shrink-0">
            <span>Soporte Express de unificación de catálogos para Margarita</span>
            <span className="font-mono text-[9.5px] text-stone-400">Constructora Arza S.A. de C.V. — 2026</span>
          </footer>
      ...
      <section className="bg-arza-50 text-center py-3 text-xs text-arza-900 font-semibold border-t border-arza-100 shrink-0 select-none">
        <p className="flex items-center justify-center space-x-1">
          <HelpCircle className="w-4 h-4 text-arza-600 mr-1.5 animate-pulse" />
